...
class TraditionalHorizonDetector:
    ...
    def __init__(self, init_all=True, canny_th1=25, canny_th2=45, Th_ROI=2, Th_slope=0.57, 
                 ...
                 hough_D_rho=2, hough_D_theta=pi/180, resize_factor=1.0,
                 roi_ratio=None):
        ...
        if init_all:
            ...
            self.hough_D_rho = hough_D_rho              # ✅ 修正：用參數，不是 Th_ROI
    ...
    def detect_horizon_slope_and_center(self, img, roi_ratio=None):
        """
        偵測圖像中的海平線斜率與中心點座標。
        若偵測失敗，回傳預設值 {'slope': 0.0, 'center': (中心 x, 中心 y)}。

        Returns:
            Dict: {'slope': float, 'center': (x, y)}
        """
        h, w = img.shape[:2]
        self.org_width = w
        self.org_height = h
        ...
        # === 中心座標計算（正常流程） ===
        a = (self.ye_hl - self.ys_hl) / (self.xe_hl - self.xs_hl)  # 斜率
        b = self.ys_hl - a * self.xs_hl                            # 截距
        x_center = w // 2
